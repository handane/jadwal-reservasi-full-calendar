...
<body>
	...
	<style>
		...
		.box-divider--light-all-2 {
			box-shadow: inset 0 0 0 2px rgba(68, 233, 68, 0.74);
		}
		...
		.box-background--gray100 {
			background-color: rgba(247, 197, 22, 0.97);
		}

		.box-background--cyan200 {
			background-color: rgba(68, 233, 68, 0.74);
		}
					...
					<div class="formbg">
						<center class="formbg-inner padding-horizontal--48">
							...
							if (isset($_POST["submit"])) {
								...
								if ($email != "" && $password != "") {
									$admin = mysqli_query($conn, "SELECT * FROM admin WHERE name = '$email' AND password = '$password'");
									$akunyangcocokadmin = mysqli_num_rows($admin);
									$ambil = $conn->query("SELECT * FROM user WHERE email = '$email' AND password = '$password'");
									$akunyangcocok = $ambil->num_rows;
									if ($akunyangcocok == 1) {
										$akun = $ambil->fetch_assoc();
										$_SESSION["user"] = $akun;
										echo "<script>location='index.php';</script>";
									}if($akunyangcocokadmin == 1){
										$ambiladmin = mysqli_fetch_assoc($admin);
										$_SESSION["admin"] = $ambiladmin;
